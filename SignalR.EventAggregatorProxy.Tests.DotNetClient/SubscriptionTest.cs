...
namespace SignalR.EventAggregatorProxy.Tests.DotNetClient
{
    ...
    public abstract class SubscriptionTest<TEvent> : DotNetClientTest where TEvent : class
    {
        ...
        protected virtual IEnumerable<IConstraintInfo> GetConstraintInfos()
        {
        ...
        public void Context()
        {
            ...
            var constraint = GetConstraintInfos();

            var eventAggregator = new EventAggregator<Event>()
                .Init("foo");

            for (int i = 0; i < 2; i++)
                eventAggregator.Subscribe(Mock<IHandle<TEvent>>(), constraint);
    ...
    [TestClass]
    public class When_subscribing_to_multiple_constrained_events_of_same_type : SubscriptionTest<GenericEvent<string>>
    {
        protected override IEnumerable<IConstraintInfo> GetConstraintInfos()
        {
